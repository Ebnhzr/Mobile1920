...
import com.github.ebnhzr.movietrackr.databinding.FragmentMovieUpdateBinding
        ...
        movieUpdateViewModel.navigateToMovieTracker.observe(this, Observer {
            status -> status?.let {
                when (status) {
                    ...
                    "emptyKey" -> Toast.makeText(this.context, "Enter the movie key for the update.", Toast.LENGTH_SHORT).show()
                    "emptyMovieName" -> Toast.makeText(this.context, "The movie should have a title.", Toast.LENGTH_SHORT).show()
                    "emptyRating" -> Toast.makeText(this.context, "Please rate the movie first.", Toast.LENGTH_SHORT).show()
                    "movieNotFound" -> Toast.makeText(this.context, "There is no movie with that kind of Movie Key.", Toast.LENGTH_SHORT).show()
                }
            }
        })
